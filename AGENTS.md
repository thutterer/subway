...
| `npx playwright test e2e/` | Run 17 e2e tests (~12s) |
...
- **Block management**: `edit-page.ts` renders per-position inline inserters between every block (and top/bottom). A `+` pill expands to `Text`/`List`/`---` buttons. Each block has a `-` delete button (absolute top-right, hidden on pointer devices via `@media (hover: hover)`, shown on `.block-group:hover`). Block groups are wrapped in `.block-group` with hover background. All mutations update `this._blocks` immediately and persist via `dbUpdateDoc`.
...
- **`note-item` → `block-changed` → `edit-page._onBlockChanged`** — note-item dispatches keystroke events; edit-page updates blocks in DB (awaited for consistency).
...
## Dexie Cloud addon
Conditionally loaded in `db.ts` when `VITE_DB_URL` is set. Without it, `db.cloud` is `undefined` and `index-page.ts` guards all cloud access with optional chaining. Schema uses `@id` (cloud) vs `id` (core Dexie). `dbCreateDoc` provides `crypto.randomUUID()` for every doc.

## Test patterns
- **17 e2e tests** in `e2e/spec.spec.ts` covering Index, Notes (CRUD, title edit, UI delete), Lists (CRUD, toggle, task delete, progress bar), Blocks (insert text/list/divider, delete block, display type), and Navigation (back-link).
