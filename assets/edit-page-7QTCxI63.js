...
`)}function h(e){return e.replace(/`([^`\n]+)`/g,'<span class="md-code">`$1`</span>').replace(/\*\*(.*?)\*\*/g,`<span class="md-bold">**$1**</span>`).replace(/__(.*?)__/g,`<span class="md-bold">__$1__</span>`).replace(/(?<!\*)\*([^*\n]+)\*(?!\*)/g,`<span class="md-italic">*$1*</span>`).replace(/(?<!_)_([^_\n]+)_(?!_)/g,`<span class="md-italic">_$1_</span>`).replace(/~~(.*?)~~/g,`<span class="md-strike">~~$1~~</span>`).replace(/\[([^\]]+)\]\(([^)]+)\)/g,`<span class="md-link">[$1]($2)</span>`).replace(/!\[([^\]]*)\]\(([^)]+)\)/g,`<span class="md-image">![$1]($2)</span>`)}var g=class extends e{static{this.properties={noteId:{},text:{type:String}}}static{this.styles=r`
    .editor-wrapper {
      display: grid;
  ...
  `}_onInput(e){let t=e.target.value;n(this.noteId,t)}render(){return t`
    ...
    `}};customElements.define(`note-item`,g);var _=class extends e{constructor(...e){super(...e),this.tasks=[]}static{this.properties={noteId:{},tasks:{type:Array}}}static{this.styles=r`
    :host {
      display: block;
    }
...
`).map(e=>e.trim()).filter(Boolean);if(n.length===0||n.length>1&&!confirm(`${n.length} items will be added`))return;for(let e of n)this.tasks=[...this.tasks,{id:crypto.randomUUID(),text:e,done:!1}];this._dispatch();let r=this.renderRoot.querySelector(`input`);r.value=``}_toggleTask(e){this.tasks=this.tasks.map(t=>t.id===e?{...t,done:!t.done}:t),this._dispatch()}_deleteTask(e,t){e.stopPropagation(),this.tasks=this.tasks.filter(e=>e.id!==t),this._dispatch()}_dispatch(){this.dispatchEvent(new CustomEvent(`list-changed`,{detail:{id:this.noteId,tasks:this.tasks},bubbles:!0,composed:!0}))}render(){let e=this.tasks.filter(e=>e.done).length;return t`
    ...
    `}};customElements.define(`list-item`,_);var v=class extends e{constructor(...e){super(...e),this.noteId=``,this.title=``,this.type=``,this.text=``,this._tasks=[],this._editing=!1}static{this.properties={noteId:{},title:{type:String},text:{type:String},type:{type:String},_editing:{state:!0}}}static{this.styles=r`
    .header {
      display: flex;
      align-items: center;
      gap: 0.5rem;
  ...
  `}connectedCallback(){super.connectedCallback(),this.noteId&&this._subscribe(this.noteId)}disconnectedCallback(){super.disconnectedCallback(),this._sub?.unsubscribe()}_subscribe(e){this._sub?.unsubscribe(),this._sub=c(()=>s.notes.get(e)).subscribe({next:e=>{e&&(this.title=e.title??``,this.text=e.text,this.type=e.type??``,this._tasks=e.tasks??[])}})}willUpdate(e){e.has(`noteId`)&&this._subscribe(this.noteId)}async _delete(){confirm(`Delete this note?`)&&(await a(this.noteId),this.dispatchEvent(new CustomEvent(`navigate`,{bubbles:!0,composed:!0,detail:{path:``}})))}_startEdit(){this._editing=!0,this.updateComplete.then(()=>{this.renderRoot.querySelector(`.title-input`)?.focus()})}async _save(e){let t=e.target;this.title=t.value,this._editing=!1,await n(this.noteId,this.text,this._tasks,this.title)}_onKeyDown(e){(e.key===`Enter`||e.key===`Escape`)&&e.target.blur()}async _onListChanged(e){let{tasks:t}=e.detail;this._tasks=t,await n(this.noteId,this.text,t,this.title)}render(){return t`
      <div class="header">
        <back-link></back-link>
    ...
    `}};customElements.define(`edit-page`,v);